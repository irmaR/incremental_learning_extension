function [sampleList,values,Dist,K,current_sample,current_labels] = MAED_incremental_1(original_sample,original_sample_class,new_points,new_classes,D,selectNum,options)
...
fprintf('original samples')

[Dist,AugSample,gnd]=EuDist2_increm_1(original_sample,original_sample_class,D,new_points,new_classes,0);
K = constructKernel_incremental(Dist,options);
nSmp = size(original_sample,1)+size(new_points,1);
options.gnd=gnd;
...
if isfield(options,'ReguBeta') && options.ReguBeta > 0
    if isfield(options,'W')
        ...
        W = constructW(AugSample,Woptions);
...
switch lower(options.Method)
    case {lower('Seq')} 
        ...
        current_sample=AugSample(sampleList,:);
        current_labels=gnd(sampleList,:);
        Dist=EuDist2(current_sample,[],0);
        K = constructKernel_incremental(Dist,options);
    otherwise
...
function [D]=update_D(D,indices_to_remove)
  remaining_indices=setdiff([1:1:size(D,1)],indices_to_remove);
  
  D=D(remaining_indices,remaining_indices);
end
