function [train,class,test_fea,binary_classes_test]=prepare_data(positive_class,smpRank,train_fea,train_class,test_fea,test_class)
...
for i = 1:length(smpRank)      
    if binary_classes_train{i,1}=='pos'
        positive_selected(counter_pos,:)=train_fea(smpRank(i),:);
        positive_class_sel(counter_pos,:)=binary_classes_train(i);
        counter_pos=counter_pos+1;
    else
        negative_selected(counter_neg,:)=train_fea(smpRank(i),:);
