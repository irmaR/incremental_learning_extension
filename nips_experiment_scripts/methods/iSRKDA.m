        ...
        for j=1:length(settings.kernelParams)
            for b=1:length(settings.reguBetaParams)
                ...
                for k=1:length(folds)
                    ...
                    [res]=MAEDIncremental(train_batch,train_batch_class,nr_samples,batch_size,options,report_points_up,data_limit,experiment_name,warping,inferenceType);
                    aucs=[];
...
[res]=MAEDIncremental(settings.XTrain,settings.YTrain,settings.numSelectSamples,settings.batchSize,options,settings.reportPoints,settings.balanced,inferenceType);
